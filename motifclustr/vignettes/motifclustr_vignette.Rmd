...
This vignette demonstrates how to use the R package `motifclustr`.
These methods are detailed in the paper
*Motif-Based Spectral Clustering of Weighted Directed Networks*,
which is available at [`arXiv:2004.01293`](arxivpaper).
The functionality of the `motifclustr` package falls into a few main categories:
...
```{r}
build_motif_adjacency_matrix(G1, "M1")
```
...
```{r}
build_motif_adjacency_matrix(G1, "M1", motif_type = "func")
```
...
```{r}
build_motif_adjacency_matrix(G1, "M1", motif_type = "func", weight_type = "mean")
```
...
```{r}
build_motif_adjacency_matrix(G1, "M1", motif_type = "func", weight_type = "product")
```
...
```{r}
mam_sparse <- build_motif_adjacency_matrix(G1, "M1", mam_method = "sparse")
mam_dense  <- build_motif_adjacency_matrix(G1, "M1", mam_method = "dense")
...
sample_dsbm(block_sizes, connection_matrix, weight_matrix, weight_type = "constant")
```
...
```{r}
sample_dsbm(block_sizes, connection_matrix, weight_matrix, weight_type = "poisson")
```
...
```{r}
spectrum = run_laplace_embedding(G2, 2, "rw")
spectrum$vects
spectrum$vals
```
...
```{r}
spectrum = run_motif_embedding(G3, "M1", "func", "unweighted", "sparse", 2, "rw")
spectrum$vals
spectrum$vects
```
...
The overall aim of `motifclustr` is to use motifs for spectral clustering,
